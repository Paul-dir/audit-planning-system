...
export const Textarea = forwardRef(({ label, error, helper, className = '', ...props }, ref) => (
  <div className="space-y-1">
    ...
    <textarea
      ref={ref}
      rows={4}
      className={`block w-full rounded-lg border ${error ? 'border-red-300 focus:ring-red-400' : 'border-gray-300 focus:ring-blue-500'} px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent bg-white text-gray-900 resize-none ${className}`}
      {...props}
    />
    {error && <p className="text-xs text-red-600">{error}</p>}
    {helper && !error && <p className="text-xs text-gray-500">{helper}</p>}
