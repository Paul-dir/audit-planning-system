...
export default function DirectorDashboard({ view }) {
  ...
  const approved = state.plans.filter(p =>
    ['DIRECTOR_APPROVED','AWAITING_REGIONAL_FEEDBACK','FEEDBACK_COLLECTED',
     'AMENDMENT_REQUIRED','SUBMITTED_TO_SENIOR_MGMT','SENIOR_MGMT_APPROVED','FINALIZED'].includes(p.status)
  );
    ...
    ...(showActions ? [{
      key: '_act', label: '', render: (_, row) => (
        <div className="flex gap-1.5 justify-end flex-wrap" onClick={e => e.stopPropagation()}>
          ...
          {/* SUBMITTED_TO_DIRECTOR: approve or request revision */}
          ...
          {/* DIRECTOR_APPROVED: send to regions */}
          ...
          {/* FEEDBACK_COLLECTED: send for amendment to planning team */}
          ...
          {/* AMENDMENT_REQUIRED back to submitted: submit to senior mgmt */}
          ...
          {/* SUBMITTED_TO_SENIOR_MGMT or FEEDBACK_COLLECTED direct path */}
          {row.status === 'FEEDBACK_COLLECTED' && (
            <Button size="xs" variant="indigo" icon={ArrowRight} onClick={() => openAction(row, 'submit_senior')}>
              → Senior Mgmt
            </Button>
    ...
    <div className="space-y-6">
      ...
      {pending.length > 0 && (
        <Alert type="info" title={`${pending.length} plan${pending.length > 1 ? 's' : ''} awaiting your review`}>
          Approve or request revisions on submitted audit plans.
        </Alert>
      )}
      {feedbackCollected.length > 0 && (
        <Alert type="warning" title={`${feedbackCollected.length} plan${feedbackCollected.length > 1 ? 's have' : ' has'} all regional feedback collected`}>
          Review the regional feedback and send the plan back to the planning team for amendment, or submit directly to Senior Management.
        </Alert>
      )}
      {seniorApproved.length > 0 && (
        <Alert type="success" title={`${seniorApproved.length} plan${seniorApproved.length > 1 ? 's' : ''} approved by Senior Management`}>
          Click <strong>"Send Approved to Regions"</strong> to deploy the plan — this will distribute cases to all regions and tax centers.
        </Alert>
      )}
          ...
          <div className="space-y-4">
            ...
            {actionType === 'amendment' && reviewPlan.regionalFeedback && (
              <div className="space-y-2">
                <p className="text-sm font-semibold text-gray-700">Regional Feedback Summary</p>
                {Object.entries(reviewPlan.regionalFeedback).map(([region, fb]) => (
                  <div key={region} className="bg-gray-50 rounded-lg p-3 text-xs">
                    <p className="font-medium text-gray-700 capitalize mb-1">{region.replace(/_/g,' ')}</p>
